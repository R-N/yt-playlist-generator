...
- **Tests** (run from `review_app/backend`): `python -m unittest discover -p "test_*.py"` (`test_db.py` data layer incl. the NaN-scrub in `_expand_extra`, `test_api.py` endpoints via FastAPI TestClient — needs `httpx` — incl. the `/api/yt_audio` redirect and the NaN-serialization guard on `/api/rows`, `test_integrations.py` settings/.env + Discord service + job catalog with network stubbed); frontend `cd frontend && npm run test` (Vitest over the pure helpers in `src/review.js`). Root scripts also have tests (run from repo root): `test_filter.py`, `test_acoustid.py`, `test_discord.py`, `test_searcher.py` (score/parse_title/helpers), `test_mb_enrich.py` + `test_lyrics.py` + `test_tag_enrich.py` (MusicBrainz + lyrics enrich; network stubbed, tag/lyrics embed roundtrips are ffmpeg-gated), `test_playlist.py` (chunking tail + id extraction), `test_url_extractor.py`, `test_cleanup_downloads.py`, and — for the two **destructive** deleters — `test_cleanup_tracked.py` + `test_check_untracked.py`, which pin the `check==1` safety gate so only verified files are ever removed. Plus `review_app/test_scripts.py` for the launchers. Not unit-tested (integration surface): the yt-dlp search/download pipeline (`searcher`/`downloader`), the Discord/network fetchers, and the best-effort lyrics scrapers; a few root scripts also carry dead helpers (`get_metadata`/`save_to_csv` in the `cleanup_*`/`check_*` files) that `main()` never calls. Network wrappers are stubbed; the yt-dlp search/download pipeline and the best-effort lyrics scrapers are intentionally left as integration surface.
